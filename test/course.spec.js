...
describe('测试课程的智能',()=>{
  ...
  it('测试添加课程',async()=>{
    ...
    assert.strictEqual(oldaddress.length,0)
    await courseList.methods.createCourse(
      "Franky's React Courses",
    ...
    assert.strictEqual(address.length,1)
  ...
  it("添加课程的属性",async ()=>{
    ...
    assert.strictEqual(name,'蜗牛的React课程')
    assert.strictEqual(content,'React+redux+reactrouter4开发招聘app')
    assert.strictEqual(target, web3.utils.toWei('8'))
    assert.strictEqual(fundingPrice, web3.utils.toWei('2'))
    assert.strictEqual(price, web3.utils.toWei('4'))
    assert.strictEqual(img,"图片的hash")
    assert.ok(!isOnline)
  ...
  it("只能ceo能删", async ()=>{
    ...
    const address = await courseList.methods.getCourse().call()
    assert.strictEqual(address.length,2)
    ...
    assert.strictEqual(address1.length,1)
  ...
  it('金钱转换',()=>{
    assert.strictEqual(web3.utils.toWei('2'),'2000000000000000000')
  })
  it('课程购买',async()=>{
  ...
  it('众筹上线后，钱到账',async()=>{
    ...
    const newBlance = new BigNumber(await web3.eth.getBalance(accounts[0]))
    const diff = newBlance.minus(oldBlance)
    assert.strictEqual(diff,web3.utils.toWei('8'))
  })
  it('课程必须是线上的价格',async()=>{
    try{
      await course.methods.buy().send({
  ...
  it('上线之后购买 有分成收益',async()=>{
    ...
    assert.strictEqual(diffCeo, web3.utils.toWei('0.4'))
    assert.strictEqual(diffOwner, web3.utils.toWei('3.6'))
    // }catch(e){
    //   assert.strictEqual(e.name,'RuntimeError')
    // }
  })
  ...
  it('上线之后，可以上传视频',async()=>{
    ...
    assert.strictEqual(video,'video的hash')
  })
})